...
impl Ability for ProjectileAbility {
    ...
    fn execute(&self, context: &mut CastContext<'_>) -> CastOutput {
        ...
        let (target_x, target_y) = bounded_target(
            player_x,
            player_y,
            context.target_x,
            context.target_y,
            self.range.min(1_000.0),
        );
        ...
        for index in 0..count {
            let offset = if count == 1 {
                ...
                (index as f64 / (count - 1) as f64 - 0.5) * self.spread_radians
            };
            ...
            let (x, y) = if self.target_origin {
                ...
                (
                    player_x + shot_angle.cos() * 22.0,
                    player_y + shot_angle.sin() * 22.0,
                )
            };
            let (kind, visual_offset_y) = match self.kind {
                ProjectileKind::Bullet => ("bullet", 0.0),
                ProjectileKind::SlashWave => ("slash_wave", 0.0),
                ProjectileKind::MagicOrb => {
                    ("magic_orb", if self.target_origin { -180.0 } else { 0.0 })
                }
            };
        ...
        let destination = (
            player_x
                + angle.cos()
                    * self
                        .distance
                        .min((target_x - player_x).hypot(target_y - player_y)),
            player_y
                + angle.sin()
                    * self
                        .distance
...
static GUNSLINGER: [ProjectileAbility; 3] = [
    ...
    ProjectileAbility {
        ...
        kind: ProjectileKind::Bullet,
        count: 9,
        spread_radians: 70.0_f64.to_radians(),
        speed: 22.0,
...
static SWORDMASTER: [ProjectileAbility; 3] = [
    ProjectileAbility {
        ...
        kind: ProjectileKind::SlashWave,
        count: 8,
        spread_radians: std::f64::consts::TAU,
        speed: 14.0,
...
static CYBERMAGE_DASH: DashAbility = DashAbility {
    ...
    fn primary_shot_uses_server_weapon_pattern() {
        ...
        assert!(
            output
                .projectiles
                .iter()
                .all(|projectile| projectile["ownerId"] == "miku")
        );
    }
}
